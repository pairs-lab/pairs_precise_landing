...
private:
  ...
  pairs_lib::SubscribeHandler<apriltag_ros::AprilTagDetectionArray> sh_tag_detections_;

  pairs_lib::PublisherHandler<geometry_msgs::PoseWithCovarianceStamped> ph_pose_;
  pairs_lib::PublisherHandler<geometry_msgs::PoseWithCovarianceStamped> ph_measurement_;
...
void LandingPadEstimation::onInit() {
  ...
  pairs_lib::ParamLoader param_loader(nh_, "LandingPadEstimation");
  ...
  sh_tag_detections_ =
      pairs_lib::SubscribeHandler<apriltag_ros::AprilTagDetectionArray>(shopts, "tag_detections_in", &LandingPadEstimation::callbackTagDetections, this);

  // | ----------------------- publishers ----------------------- |

  ph_pose_        = pairs_lib::PublisherHandler<geometry_msgs::PoseWithCovarianceStamped>(nh_, "estimated_pose_out", 10);
  ph_measurement_ = pairs_lib::PublisherHandler<geometry_msgs::PoseWithCovarianceStamped>(nh_, "measurement_pose_out", 10);
  ...
  transformer_ = pairs_lib::Transformer("LandingPadEstimation");
...
void LandingPadEstimation::publish() {
  ...
  if (time_last_correction == ros::Time::UNINITIALIZED || (ros::Time::now() - time_last_correction).toSec() > _correction_timeout_) {
    ...
    pairs_lib::set_mutexed(mutex_statecov_, {}, statecov_);
...
PLUGINLIB_EXPORT_CLASS(pairs_landing_pad_estimation::LandingPadEstimation, nodelet::Nodelet)
